...
QSize BubbleView::getRealString(QString src)
{
    ...
    if(cnt == 0){
        maxWidth = fm.horizontalAdvance(src);
        QString value = src;
        if(maxWidth > textWidth){
            maxWidth = textWidth;
            int size = textWidth/fm.horizontalAdvance(" ");
            int num = fm.horizontalAdvance(value)/textWidth;
            num = (fm.horizontalAdvance(value))/textWidth;
            cnt += num;
            QString tmp = "";
            for(int i = 0;i < num;i++){
        ...
        for(int i = 0;i < cnt + 1;i++){
            ...
            maxWidth = fm.horizontalAdvance(value) > maxWidth ? fm.horizontalAdvance(value) : maxWidth;
            if(fm.horizontalAdvance(value) > textWidth){
                maxWidth = textWidth;
                int size = textWidth/fm.horizontalAdvance(" ");
                int num = fm.horizontalAdvance(value)/textWidth;
                num = ((i+num)*fm.horizontalAdvance(" ") + fm.horizontalAdvance(value))/textWidth;
                cnt += num;
                QString tmp = "";
                for(int j = 0;j < num;j++){
                    tmp += value.mid(i*size,(i+1)*size) + "\n";
